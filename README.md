...
- put an **`AGENTS.md`** file at the project root, it is the project's operating map, a glance and any agent should have the context to work on any task for the project.
- keep a **`docs/`** folder for project context, plans, decisions, eval definitions, and quality criteria.
- keep a **`skills/`** folder for reusable units of know-how. Specify skills using the open [Agent Skills standard](https://agentskills.io/home)
- recipes are in a skill folder with more customization and specific guidelines on using the skill based on use-cases.

Use YAML for human-authored manifests, JSON Schema for machine-checked contracts, and code for execution.
